...
for i in tqdm(range(rescaled_Predicted_price.shape[0])):
    y_predict = pd.DataFrame(rescaled_Predicted_price[i], columns=["predicted_price"], index=train_predict_index[i:i+output_dim])
    predict_result = pd.concat([predict_result, y_predict], axis=1, sort=False)

real_price = pd.DataFrame()
for i in tqdm(range(rescaled_Real_price.shape[0])):
...
print("mean_absolute_error:", mean_absolute_error(real, predicted))
print("mean_squared_error:", mean_squared_error(real, predicted))
print("rmse:", sqrt(mean_squared_error(real, predicted)))
print("r2 score:", r2_score(real, predicted))
