...
def basic_GRU(input_dim, output_dim, feature_size) -> tf.keras.models.Model:
    ...
    model.compile(optimizer=Adam(lr=LR, clipnorm=0.1), loss='mse')
...
def plot_traindataset_result(X_train, y_train):
    ...
    for i in tqdm(range(rescaled_predicted_y.shape[0])):
        y_predict = pd.DataFrame(rescaled_predicted_y[i], columns=["predicted_price"],
                                 index=train_predict_index[i:i + output_dim])
        predict_result = pd.concat([predict_result, y_predict], axis=1, sort=False)

    real_price = pd.DataFrame()
    for i in tqdm(range(rescaled_real_y.shape[0])):
    ...
    print("RMSE:", sqrt(mean_squared_error(predicted, real)))
    print("R2 score:", r2_score(predicted, real))
